...
Singleton {
    ...
    Process {
        id: serverProcess
        command: [root._scriptDir + "/../.venv/bin/python3",
            root._scriptDir + "/anime_server.py"]
        running: true
        onExited: (code) => {
    ...
    function _normaliseShow(s) {
        ...
        var en = s.english_name || s.englishName || ""
        return {
            id:          s.id          || "",
            name:        root._decode(en || s.name || ""),
            englishName: root._decode(en),
            nativeName:  root._decode(s.native_name  || s.nativeName  || ""),
            thumbnail:   s.thumbnail   || "",
            score:       s.score       !== undefined ? s.score : null,
            type:        s.type        || "",
            episodeCount: s.episode_count || "",
            description: root._decode(s.description || ""),
            availableEpisodes: {
                sub: avail.sub || 0,
                dub: avail.dub || 0,
                raw: avail.raw || 0
    ...
    function _parseAnimeDetail(show, json) {
        try {
            ...
            currentAnime = Object.assign({}, show, {
                ...
                description: data.description || show.description || "",
                thumbnail: data.thumbnail || show.thumbnail || "",
                score: data.score !== undefined ? data.score : show.score
            })
            detailError = ""
        } catch (e) {
